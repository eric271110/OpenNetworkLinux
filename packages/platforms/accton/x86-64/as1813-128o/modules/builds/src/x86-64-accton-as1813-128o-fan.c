...
static ssize_t show_threshold(struct device *dev, struct device_attribute *da,
            char *buf);
static int as1813_128o_fan_probe(struct platform_device *pdev);
static int as1813_128o_fan_remove(struct platform_device *pdev);
...
struct as1813_128o_fan_data *data = NULL;

static struct platform_driver as1813_128o_fan_driver = {
    .probe = as1813_128o_fan_probe,
    .remove = as1813_128o_fan_remove,
    .driver = {
        .name = DRVNAME,
        .owner = THIS_MODULE,
    },
...
enum as1813_128o_fan_sysfs_attrs {
    FAN_ATTR(1),
    FAN_ATTR(2),
    FAN_ATTR(3),
    FAN_ATTR(4),
...
static struct attribute *as1813_128o_fan_attrs[] = {
    /* fan attributes */
    DECLARE_FAN_ATTR(1),
    DECLARE_FAN_ATTR(2),
    DECLARE_FAN_ATTR(3),
...
static struct as1813_128o_fan_data *as1813_128o_fan_update_cpld_ver(void)
{
    int status = 0;
...
static int __init as1813_128o_fan_init(void)
{
    int ret;

    data = kzalloc(sizeof(struct as1813_128o_fan_data), GFP_KERNEL);
    if (!data) {
        ret = -ENOMEM;
        goto alloc_err;
    }
...
MODULE_DESCRIPTION("as1813_128o_fan driver");
MODULE_LICENSE("GPL");

module_init(as1813_128o_fan_init);
module_exit(as1813_128o_fan_exit);
