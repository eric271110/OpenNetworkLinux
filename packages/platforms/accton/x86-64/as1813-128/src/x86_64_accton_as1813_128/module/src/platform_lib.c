...
int onlp_get_psu_hwmon_idx(int pid)
{
    ...
    for (hwmon_idx = 0; hwmon_idx <= max_hwmon_idx; hwmon_idx++) {
        snprintf(path, sizeof(path), "/sys/devices/platform/as1813_128_psu.%d/hwmon/hwmon%d/", pid-1, hwmon_idx);
...
int onlp_get_fan_hwmon_idx(void)
{
    ...
    for (hwmon_idx = 0; hwmon_idx <= max_hwmon_idx; hwmon_idx++) {
        snprintf(path, sizeof(path), "/sys/devices/platform/as1813_128_fan/hwmon/hwmon%d/", hwmon_idx);
...
as1813_128_platform_id_t get_platform_id(void)
{
    int ret, pid = 0;

    ret = onlp_file_read_int(&pid, "/sys/devices/platform/as1813_128_fpga/platform_id");
    if (ONLP_STATUS_OK != ret) {
        return PID_UNKNOWN;
    }

    return (pid == 0) ? AS1813_128O : AS1813_128D;
}
