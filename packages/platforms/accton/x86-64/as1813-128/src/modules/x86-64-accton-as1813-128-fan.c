...
static ssize_t show_threshold(struct device *dev, struct device_attribute *da,
            char *buf);
static int as1813_128_fan_probe(struct platform_device *pdev);
static int as1813_128_fan_remove(struct platform_device *pdev);
...
struct as1813_128_fan_data *data = NULL;

static struct platform_driver as1813_128_fan_driver = {
    .probe = as1813_128_fan_probe,
    .remove = as1813_128_fan_remove,
    .driver = {
        .name = DRVNAME,
        .owner = THIS_MODULE,
    },
...
enum as1813_128_fan_sysfs_attrs {
    FAN_ATTR(1),
    FAN_ATTR(2),
    FAN_ATTR(3),
    FAN_ATTR(4),
...
ATTRIBUTE_GROUPS(as1813_128_fan);
...
static struct as1813_128_fan_data *as1813_128_fan_update_device(void)
{
    int status = 0;
                            ...
                            char *buf)
{
    ...
    data = as1813_128_fan_update_device();
    if (!data->valid) {
        error = -EIO;
        goto exit;
    }
...
static struct as1813_128_fan_data *as1813_128_fan_update_cpld_ver(void)
{
    int status = 0;
                                ...
                                char *buf)
{
    ...
    data = as1813_128_fan_update_cpld_ver();
    if (!data->valid) {
        error = -EIO;
        goto exit;
    }
...
static int as1813_128_fan_probe(struct platform_device *pdev)
{
    ...
    hwmon_dev = hwmon_device_register_with_info(&pdev->dev, DRVNAME,
                    NULL, NULL, as1813_128_fan_groups);
...
static int __init as1813_128_fan_init(void)
{
    int ret;

    data = kzalloc(sizeof(struct as1813_128_fan_data), GFP_KERNEL);
    if (!data) {
        ret = -ENOMEM;
        goto alloc_err;
    }
    ...
    ret = platform_driver_register(&as1813_128_fan_driver);
    if (ret < 0)
        goto dri_reg_err;
...
static void __exit as1813_128_fan_exit(void)
{
    if (data) {
...
module_init(as1813_128_fan_init);
module_exit(as1813_128_fan_exit);
