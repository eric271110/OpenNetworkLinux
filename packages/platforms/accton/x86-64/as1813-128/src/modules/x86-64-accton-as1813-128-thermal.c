...
struct as1813_128_thermal_data {
...
static struct platform_driver as1813_128_thermal_driver = {
    .probe = as1813_128_thermal_probe,
    .remove = as1813_128_thermal_remove,
    .driver = {
        .name = DRVNAME,
        .owner = THIS_MODULE,
    },
...
static int as1813_128_thermal_probe(struct platform_device *pdev)
{
    ...
    hwmon_dev = hwmon_device_register_with_info(&pdev->dev, DRVNAME, 
                    NULL, NULL, as1813_128_thermal_groups);
...
static int __init as1813_128_thermal_init(void)
{
    int ret;

    data = kzalloc(sizeof(struct as1813_128_thermal_data), GFP_KERNEL);
    if (!data) {
        ret = -ENOMEM;
        goto alloc_err;
    }
    ...
    ret = platform_driver_register(&as1813_128_thermal_driver);
    if (ret < 0)
        goto dri_reg_err;
...
dev_reg_err:
    platform_driver_unregister(&as1813_128_thermal_driver);
dri_reg_err:
    kfree(data);
alloc_err:
    return ret;
...
static void __exit as1813_128_thermal_exit(void)
{
    if (data) {
...
module_init(as1813_128_thermal_init);
module_exit(as1813_128_thermal_exit);
