...
static ssize_t show_led(struct device *dev, struct device_attribute *attr,
            char *buf);
static int as1813_128_led_probe(struct platform_device *pdev);
static int as1813_128_led_remove(struct platform_device *pdev);
...
struct as1813_128_led_data *data = NULL;

static struct platform_driver as1813_128_led_driver = {
    .probe = as1813_128_led_probe,
    .remove = as1813_128_led_remove,
    .driver = {
        .name = DRVNAME,
        .owner = THIS_MODULE,
    },
...
enum as1813_128_led_sysfs_attrs {
    LED_LOC,
    LED_DIAG,
    LED_ALARM,
    LED_FAN,
...
static const struct attribute_group as1813_128_led_group = {
    .attrs = as1813_128_led_attributes,
};
...
static struct as1813_128_led_data *as1813_128_led_update_device(void)
{
    int status = 0;
                            ...
                            char *buf)
{
    ...
    data = as1813_128_led_update_device();
    if (!data->valid) {
        error = -EIO;
        goto exit;
    }
...
static int as1813_128_led_probe(struct platform_device *pdev)
{
    ...
    status = sysfs_create_group(&pdev->dev.kobj, &as1813_128_led_group);
    if (status)
        goto exit;
...
static int as1813_128_led_remove(struct platform_device *pdev)
{
    sysfs_remove_group(&pdev->dev.kobj, &as1813_128_led_group);

    return 0;
}

static int __init as1813_128_led_init(void)
{
    int ret;

    data = kzalloc(sizeof(struct as1813_128_led_data), GFP_KERNEL);
    if (!data) {
        ret = -ENOMEM;
        goto alloc_err;
    }
    ...
    ret = platform_driver_register(&as1813_128_led_driver);
    if (ret < 0)
        goto dri_reg_err;
...
dev_reg_err:
    platform_driver_unregister(&as1813_128_led_driver);
dri_reg_err:
    kfree(data);
alloc_err:
    return ret;
...
static void __exit as1813_128_led_exit(void)
{
...
module_init(as1813_128_led_init);
module_exit(as1813_128_led_exit);
