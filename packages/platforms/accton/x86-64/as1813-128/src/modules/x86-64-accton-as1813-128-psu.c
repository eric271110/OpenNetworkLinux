...
static ssize_t show_string(struct device *dev, struct device_attribute *attr,
                            char *buf);
static int as1813_128_psu_probe(struct platform_device *pdev);
static int as1813_128_psu_remove(struct platform_device *pdev);
...
struct as1813_128_psu_data *data = NULL;

static struct platform_driver as1813_128_psu_driver = {
    .probe = as1813_128_psu_probe,
    .remove = as1813_128_psu_remove,
    .driver = {
        .name = DRVNAME,
        .owner = THIS_MODULE,
    },
...
enum as1813_128_psu_sysfs_attrs {
    /* psu attributes */
    PSU_ATTR(1),
    PSU_ATTR(2),
    NUM_OF_PSU_ATTR,
...
static struct attribute *as1813_128_psu1_attrs[] = {
    /* psu attributes */
    DECLARE_PSU_ATTR(1),
...
static struct attribute_group as1813_128_psu1_group = {
    .attrs = as1813_128_psu1_attrs,
};
/* ATTRIBUTE_GROUPS(as1813_128_psu1); */
...
static struct attribute *as1813_128_psu2_attrs[] = {
    /* psu attributes */
    DECLARE_PSU_ATTR(2),
...
static struct as1813_128_psu_data *as1813_128_psu_update_device(struct device_attribute *da)
{
                            ...
                            char *buf)
{
    ...
    data = as1813_128_psu_update_device(da);
    if (!data->valid[pid]) {
        error = -EIO;
        goto exit;
    }
...
static int as1813_128_psu_probe(struct platform_device *pdev)
{
    ...
    hwmon_dev = hwmon_device_register_with_info(&pdev->dev, DRVNAME, 
                    NULL, NULL, as1813_128_psu_groups[pdev->id]);
    if (IS_ERR(hwmon_dev)) {
...
static int __init as1813_128_psu_init(void)
{
    int ret;
    int i;

    data = kzalloc(sizeof(struct as1813_128_psu_data), GFP_KERNEL);
    if (!data) {
        ret = -ENOMEM;
        goto alloc_err;
    }
    ...
    ret = platform_driver_register(&as1813_128_psu_driver);
    if (ret < 0)
        goto dri_reg_err;
...
dev_reg_err:
    platform_driver_unregister(&as1813_128_psu_driver);
dri_reg_err:
    kfree(data);
alloc_err:
    return ret;
...
static void __exit as1813_128_psu_exit(void)
{
    int i;
...
MODULE_DESCRIPTION("as1813_128_psu driver");
MODULE_LICENSE("GPL");

module_init(as1813_128_psu_init);
module_exit(as1813_128_psu_exit);
