...
static const struct attribute_group as1813_128_sys_group = {
    .attrs = as1813_128_sys_attributes,
};
...
static int __init as1813_128_sys_init(void)
{
    int ret;

    data = kzalloc(sizeof(struct as1813_128_sys_data), GFP_KERNEL);
    if (!data) {
        ret = -ENOMEM;
        goto alloc_err;
    }
...
dev_reg_err:
    platform_driver_unregister(&as1813_128_sys_driver);
dri_reg_err:
    kfree(data);
alloc_err:
    return ret;
...
static void __exit as1813_128_sys_exit(void)
{
...
module_init(as1813_128_sys_init);
module_exit(as1813_128_sys_exit);
